...
/**
 * Entries of the allow list; a missing file yields `[]`. Malformed entries are
 * skipped. An entry without a non-blank `reason` is skipped with a warning:
 * specs/15 says both allow-listing mechanisms MUST record a reason, so a
 * reason-less entry never suppresses a finding.
 */
export async function loadAllowList(brainRoot: string): Promise<AllowEntry[]> {
  ...
  for (const item of list) {
    ...
    if (typeof item.reason !== 'string' || item.reason.trim().length === 0) {
      out.warn('ignoring allow-list entry ' + item.fingerprint + ': missing reason');
      continue;
    }
    entries.push({
      ...item,
