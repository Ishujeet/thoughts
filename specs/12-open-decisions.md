...
| D22 | Brain backends | **git is the default; PostgreSQL and NebulaGraph are opt-in standalone brains.** At `init` the user picks ONE backend as the brain of record: git (default), psql, or nebula. Each is a standalone brain; thoughts remain OKF markdown + YAML frontmatter documents in every backend, and `~/.thoughts/brains/<id>/` is the working set for all of them. The backend is immutable after init in v1. G4 of [00](00-overview.md) amended to "plain files by default, no lock-in"; the non-goals drop "no database" and keep "no SaaS, no accounts". **Representation** (refined 2026-09-17): the OKF document is the canonical *unit* in every backend, but each store MUST represent thoughts natively (psql columns + jsonb, nebula vertex properties/edges — a markdown dump is non-conforming), serving MUST use the backend's native index rather than walking files, and the workspace stays the agent's materialised markdown view in every backend. | [16-brain-backends.md](16-brain-backends.md#representation), [00](00-overview.md), [01](01-brain-repo.md) |
| D23 | Codebase graph | **web-tree-sitter (WASM), grammars as optional dependencies, fail-soft.** Languages ts, tsx, js, jsx, py, go, rs. RepoGraph v1: nodes file/symbol/module (stable hash ids, file nodes carry a content sha), edges contains/imports/calls/imports_repo, deterministic ordering. Stored per repo in the brain at `repos/<id>/codegraph/` (git: `graph.json` + `meta.yml` + generated `index.md`; pg: `codegraph_nodes`/`edges`/`meta` tables; nebula: native tags/edges). Built at `init`, updated incrementally at `sync` (>500 changed files or missing meta → full rebuild; ANY grammar/extraction failure → exactly one warning and `sync` continues). Cross-repo edges matched by module name to sibling repos' optional `package` field. Surfaced in `status`. (decided 2026-09-17) | [17](17-codegraph.md), [03](03-cli-sync.md), [04](04-cli-status.md) |
| D24 | Help | **commander built-ins, no new deps.** Grouped command list (`cmd.group()` + `formatHelp` override): getting started / daily / maintenance / planned, with unimplemented commands listed under planned as "not in this version". Global options block, `addHelpText('after')` typical-session example, per-command examples, `showSuggestionOnError()`. New `thoughts help <topic>` subcommand (topics: brain, backends, secrets, okf; unknown topic exits 1 listing topics). (decided 2026-09-17) | [18-cli-help.md](18-cli-help.md) |
...
| O4 | May a brain be migrated between backends (git → psql, psql → nebula, …)? | **Not in v1; the backend is immutable after init** (D22). The workspace-at-`~/.thoughts/brains/<id>/` layout keeps an export path feasible later if demand appears. Proposed 2026-09-17. |
